...
public class AnvilGUI {
    ...
    //protected IAnwillWrapper WRAPPER;
    ...
    public AnvilGUI(Plugin plugin, Player holder, String insert, BiFunction<Player, String, String> biFunction) {
        this( holder, "Введите текст", insert, null, null, false, null, null, null, (player, text) -> {
...
/**
    ...
    private AnvilGUI(
       ...
       /* if (WRAPPER==null) {
            try {
                ...
                Ostrov.log_err("AnvilGUI init : "+ex.getMessage());
                return;
            }
        }*/
    ...
    private void openInventory() {
        VM.anwillWrapper.handleInventoryCloseEvent(player);
        VM.anwillWrapper.setActiveContainerDefault(player);

        Bukkit.getPluginManager().registerEvents(listener, Ostrov.instance);

        final Object container = VM.anwillWrapper.newContainerAnvil(player, inventoryTitle);

        inventory = VM.anwillWrapper.toBukkitInventory(container);
        ...
        containerId = VM.anwillWrapper.getNextContainerId(player, container);
        VM.anwillWrapper.sendPacketOpenWindow(player, containerId, inventoryTitle);
        VM.anwillWrapper.setActiveContainer(player, container);
        VM.anwillWrapper.setActiveContainerId(container, containerId);
        VM.anwillWrapper.addActiveContainerSlotListener(container, player);
        open = true;
    }
    ...
    private void closeInventory(boolean sendClosePacket) {
        ...
        if (sendClosePacket) {
            VM.anwillWrapper.handleInventoryCloseEvent(player);
            VM.anwillWrapper.setActiveContainerDefault(player);
            VM.anwillWrapper.sendPacketCloseWindow(player, containerId);
        }
    ...
    private class ListenUp implements Listener {
        ...
        public void onInventoryClick(InventoryClickEvent e) {
            if ( e.getInventory().equals(inventory) && (e.getRawSlot() < 3 || e.getAction().equals(InventoryAction.MOVE_TO_OTHER_INVENTORY)) ) {
                e.setCancelled(true);
                ...
                switch (e.getRawSlot()) {
                    case Slot.OUTPUT:
                        ...
                        final Response response = completeFunction.apply(
                        	clicker, clicked.hasItemMeta() ? TCUtils.toString(clicked.getItemMeta().displayName()) : ""
                        ); 
                        if (response.getText() != null) {
                            final ItemMeta meta = clicked.getItemMeta();
                            meta.displayName(TCUtils.format(response.getText()));
        ...
        public void onInventoryClose(InventoryCloseEvent event) {
            if (open && event.getInventory().equals(inventory)) {
                ...
                if (preventClose) {
                    Bukkit.getScheduler().runTask(Ostrov.instance, AnvilGUI.this::openInventory);
                }
            }
        }
    ...
    public static class Builder {
         ...
         */
        private Plugin plugin = Ostrov.instance;
        /**
        ...
        public AnvilGUI open(Player player) {
            ...
            return new AnvilGUI(player, title, itemText, itemLeft, itemRight, preventClose, closeListener, inputLeftClickListener, inputRightClickListener, completeFunction);
        }
