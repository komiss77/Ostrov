...
public class TprCmd implements OCommand {
  ...
  private static final List<String> ALIASES = List.of("тпр", "rtp", "ртп");
...
/*
    ...
    private int perform(final CommandSender cs, final Player p, final World in, final int dst) {
        ...
        if (!force) Timer.add(p, "tpr", Cfg.tpr_command_delay);
        ...
        tpData.put(p.getName(), new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (; tryPerTick > 0; tryPerTick--) {
                    ...
                    feetLoc.y = maxY;
                    //final WXYZ lc = new LocFinder(feetLoc).find(true, 1, 1); -не надо, будет просто безопасный телепорт, он точно найдёт место
                    //if (lc == null) continue;
                    //feetLoc.x = lc.x;
                    //feetLoc.y = lc.y;
                    // feetLoc.z = lc.z;
...
//Ostrov.log(find_x+","+find_y+","+find_z+" "+downMat+"-"+feetMat+"-"+headMat);
                                //если над нижним блоком нет 2 блока для тела, пропускаем ниже
                                //if (!LocUtil.isPassable(headMat) || !LocUtil.isPassable(feetMat)) {
                                //    continue;
                                //}
                                feetLoc.setY(feet_y + 0.6);
                                //if (LocUtil.canStand(downMat) || downMat==Material.WATER) { //вода или подходит для стояния - сойдёт
                                if (TeleportLoc.isSafePlace(headMat, feetMat, downMat)) {
                                    break;
                                }
                            }/
