...
public class TpposCMD implements OCommand {
    ...
    private static Command<CommandSourceStack> executor() {
        return cntx -> {
            ...
            if (YT_GROUP == null) YT_GROUP = Perm.getGroup("youtuber");
            if (Cfg.tppos_command || op.hasGroup(YT_GROUP)) {
                if (p.hasPermission("ostrov.tppos") || op.hasGroup(YT_GROUP)) {
                    if (arg.length == 3) {
