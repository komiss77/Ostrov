...
//Ostrov.log_warn("Didnt find A* " + from + " or " + to); долбит консоль если просто взлететь вверх
...
//						Bukkit.getConsoleSender().sendMessage("c-" + curr.toString() + ", p-" + crp.toString() + ", d-" + curr.distAbs(crp));
    ...
    private static XYZ[] getNear(final World w, final Node nd, final boolean jump) {//, final Map<Integer, XYZ[]> nodes
        ...
        while (ndi.hasNext()) {
            ...
            switch (is.getFastMat(w, lc.x, lc.y - 1, lc.z)) {
                case ACACIA_FENCE, ACACIA_FENCE_GATE, BAMBOO_FENCE, BAMBOO_FENCE_GATE, BIRCH_FENCE, BIRCH_FENCE_GATE, CHERRY_FENCE, CHERRY_FENCE_GATE, CRIMSON_FENCE, CRIMSON_FENCE_GATE, DARK_OAK_FENCE, DARK_OAK_FENCE_GATE, JUNGLE_FENCE, JUNGLE_FENCE_GATE, MANGROVE_FENCE, MANGROVE_FENCE_GATE, NETHER_BRICK_FENCE, OAK_FENCE, OAK_FENCE_GATE, SPRUCE_FENCE, SPRUCE_FENCE_GATE, WARPED_FENCE, WARPED_FENCE_GATE, ANDESITE_WALL, BLACKSTONE_WALL, BRICK_WALL, COBBLED_DEEPSLATE_WALL, COBBLESTONE_WALL, DEEPSLATE_BRICK_WALL, DEEPSLATE_TILE_WALL, STONE_BRICK_WALL, RED_SANDSTONE_WALL, RED_NETHER_BRICK_WALL, POLISHED_DEEPSLATE_WALL, POLISHED_BLACKSTONE_WALL, POLISHED_BLACKSTONE_BRICK_WALL, NETHER_BRICK_WALL, MOSSY_COBBLESTONE_WALL, MOSSY_STONE_BRICK_WALL, GRANITE_WALL, END_STONE_BRICK_WALL, DIORITE_WALL:
                    ndi.remove();
                default:
                    break;
            }
    ...
    private static void lookNear(final XYZ sp, final Set<XYZ> nds, final World w, final int dx, final int dz, final boolean jump, final IServer is) {
        ...
        if (is.getFastMat(w, nxt.x, nxt.y + 1, nxt.z).isCollidable()) {
            return;//?|? B ?
        }
        if (is.getFastMat(nxt).isCollidable()) {//?|B 0 ?
            if (is.getFastMat(w, nxt.x, nxt.y + 2, nxt.z).isCollidable()) {
                return;//?|B 0 B
            }
            if (is.getFastMat(w, sp.x, sp.y + 2, sp.z).isCollidable()) {
                return;//?|B 0 v
            }
            ...
            if (is.getFastMat(w, nxt.x, nxt.y - 1, nxt.z).isCollidable()) {//B|0 0 ?
                ...
                if (is.getFastMat(w, sp.x, sp.y + 2, sp.z).isCollidable() || !jump) {
                    return;//.. 9<-.. 0|0 0 v
                }				//>0 block jump
                WXYZ jmp;
                for (int i = 2; i != 5; i++) {
    ...
    WXYZ getIfWalk(final WXYZ lc, final IServer is) {
