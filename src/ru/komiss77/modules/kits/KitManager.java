...
public final class KitManager extends Initiable {
                ...
                for (String kitName : kitsConfig.getConfigurationSection("kits").getKeys(false) ) {
                    ...
                    try {
                        for (String itemAsString :  kitsConfig.getStringList("kits."+kitName+".items") ) {
                            items.add( ItemUtils.getItemStackFromString(itemAsString, "<>"));
                            if (items.size()==28) {
    ...
    public static boolean buyKitAcces ( final Player player, final String kitName ) {
        ...
        if ( PM.Kit_has_acces(player.getName(), kitName ) ) {
        ...
        PM.Kit_add_acces(player.getName(), kitName);
    ...
    public static boolean trySellAcces ( final Player player, final String kitName ) {
        ...
        PM.Kit_remove_acces(player.getName(), kitName);
    ...
    public static boolean tryGiveKit ( final Player player, final String kitName ) {
        ...
        if ( kit.accesBuyPrice>0 && !PM.Kit_has_acces(player.getName(), kitName) ) {
...
//System.out.println("getMinLetf deley="+kit.delayMin+"  lastAccesBelow="+( Timer.Единое_время()/1000 - PM.Kit_last_acces(player.getName(), kit.name) )+
    ...
    public static void giveKit( final Player p, final String kitName, final boolean equipArmor ) {
        ...
        p.updateInventory();;
        PM.Kit_recieved(p.getName(), kitName);
    ...
    /*public static int Kit_size ( String kit ) {
        return kits.get(kit).Get_size();
    }
    public static String Kit_name_by_display_name(String display_name) {
        for (String name:GetKits() ) {
            if ( kits.get(name).Get_display_name().equals(display_name) ) return name;
        }
        return null;
    }


    // --------- Права доступа к набору ------------------
    public static boolean Is_need_perm ( String kit ) {
        return kits.get(kit).Is_need_perm();
    }
    public static String Give_need_perm ( String kit ) {
        return kits.get(kit).Give_need_perm();
    }

    public static boolean Is_need_acces_buy(String kit) {
            return kits.get(kit).Is_need_acces_buy();
        }
    public static boolean Is_need_get_payment(String kit) {
            return kits.get(kit).Is_need_get_payment();
        }
    public static int Kit_acces_cost(String kit) {
            return kits.get(kit).Kit_acces_cost();
        }
    public static int Kit_give_cost(String kit) {
            return kits.get(kit).Kit_give_cost();
        }
    public static int Kit_delay ( String kit ) {
        return kits.get(kit).Kit_delay();
    }
    public static ItemStack Kit_logo ( String kit ) {
        return kits.get(kit).Kit_logo();
    }
*/
...
//System.out.println("saveKit");        
        ...
        for (ItemStack is : kit.items) {
            itemsList.add(ItemUtils.itemStackToString(is, "<>"));
        }
