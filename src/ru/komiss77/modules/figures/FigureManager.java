...
public class FigureManager implements Initiable {
    ...
    static { //класс прогружается до загрузки миров, поэтому активируем в init после загрузки миров
        ...
        stick = new ItemBuilder(Material.STICK)
            ...
            .lore("§aПКМ на стойку или моба §7- ")
            .lore("§7создать фигуру.")
            .lore("§7Создание начинается с выбора типа.")
            .lore("§7Если тип не выбрать, фигура не создастся.")
            .lore("§7После выбора типа происходит")
            .lore("§7первое сохранение, и фигура станет активна.")
            .lore("§7")
            .lore("§aПКМ на фигуру §7- настройки")
            .lore("§7")
            .build();
...
//System.out.println(""+f.name+"!hasNearbyPlayers"); 
    ...
    protected static Figure getFigure(final Entity e) {
        if (isFigure(e))
            return figures.get(e.getPersistentDataContainer().get(key, PersistentDataType.INTEGER));//return figures.get(uuid.get(entityUUID));
        return null;
    }
