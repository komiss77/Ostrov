...
public class ScoreBoard {
	...
	public void reanimate(@Nullable final Entity dis) {
		...
		for (int i = 0; i < length; i++) {
			...
			if (nm == null) sb.append("§8" + (i + 1) + ") " + clr + "=-=-=-=§б: --\n");
			else sb.append("§8" + (i + 1) + ") " + clr + nm + "§б: " + toDisplay(stats.get(nm)) + "\n");
		}
	...
	public boolean tryAdd(final String name, final int amt) {
		...
		for (final String nm : ranks) {
			...
			if (sc == null || isAsc ? sc >= amt : sc <= amt) plc++;
			else break;
		}
		
		if (plc > length) return false;
		stats.put(name, amt);
		if (plc > ranks.size()) ranks.add(name);
		else {
		...
		if (plc == 1) new ScoreWorldRecordEvent(name, amt, this).callEvent();
		reanimate(getEntity());
		return true;
	}
