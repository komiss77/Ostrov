        ...
        for (final String friendName : op.friends) {
            if (server.containsKey(friendName)) {
                ...
                final ItemStack friend_item = new ItemBuilder(ItemType.PLAYER_HEAD)
                    ...
                    .lore(Settings.hasSettings(friendSettings, Settings.Fr_MsgDeny) ? "§8Сообщения отключены" : "§7ЛКМ - §6Написать ЛС")
                    .lore(Settings.hasSettings(friendSettings, Settings.Fr_TeleportDeny) ? "§8Запрос на ТП отключён" : "§7ПКМ - §bЗапрос на ТП")
