            ...
            case ПРОФИЛЬ -> {
                if (op.eng) {
                    lore = Arrays.asList(
                        ...
                        Component.text( Lang.t( Stat.PLAY_TIME.desc, EnumLang.EN_US)+ApiOstrov.secondToTime(op.getDaylyStat(Stat.PLAY_TIME))),
                        Component.text(Lang.t( Stat.LEVEL.desc, EnumLang.EN_US)+op.getStat(Stat.LEVEL) +"  "+ ApiOstrov.getPercentBar(op.getStat(Stat.LEVEL)*25, op.getStat(Stat.EXP), true)),
                        Component.empty(),
