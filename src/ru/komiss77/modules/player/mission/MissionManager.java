...
public class MissionManager {
    ...
    protected static int getLimit(final Oplayer op) {
        ...
        if (op.getPlayer().hasPermission("ostrov.missioner"))
            return 5; //ApiOstrov.hasGroup(op.nik, "missioner")) return 5;
                        ...
                        if (rs.getInt("completed") > 0) { //уже выполнена
                            ...
                            for (String requestName : mission.request.keySet()) {
                                ...
                                if (progressMap.containsKey(requestName)) {
                                    ...
                                    if (current >= request) {
                                        if (showAmmount) {
                                            lore.add(Component.text("§a✔ §8" + TCUtil.strip(displayName) + current + " (" + request + ")"));
                                        } else {
                                            lore.add(Component.text("§a✔ §8" + TCUtil.strip(displayName)));
                                        }
                                    } else {
                                        if (showAmmount) {
