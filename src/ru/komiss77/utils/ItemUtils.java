...
//System.out.println("2 itemstack="+itemstack);
  ...
  public static boolean compareItem(@Nullable final ItemStack is1, @Nullable final ItemStack is2, @Nullable final BiPredicate<ItemMeta, ItemMeta> metaCheck) {
    if (is1 == null || is2 == null) {
      return is1 == is2;
    }

    if (is1.getType() == is2.getType()) { //тип совпадает
      if (metaCheck == null) return true;
      if (is1.hasItemMeta() && is2.hasItemMeta()) { //если у обоих есть мета
        return metaCheck.test(is1.getItemMeta(), is2.getItemMeta());
      } else {
  ...
  public static boolean compareName(final ItemMeta im1, final ItemMeta im2) {
    return im1.hasDisplayName() && im2.hasDisplayName()
      ? TCUtils.compare(im1.displayName(), im2.displayName())
