    ...
    public static Connection GetConnection() {
        ...
        try {
            if ( connection == null || !connection.isValid(3)) {
                Disconnect();
                Ostrov.log_warn("§6MySQL - создаём ostrov подключение..."); //не ставить log_err, или зацикливает!!!
                try {
                    Class.forName("com.mysql.cj.jdbc.Driver");
                    connection = DriverManager.getConnection(url);
                } catch (SQLException | ClassNotFoundException e) {
                    Ostrov.log_warn("§cMySql: соединение с базой ostrov  не удалось !"+e.getMessage()); //не ставить log_err, или зацикливает!!!
                    connection = null;
                }
            }
        } catch (SQLException e) {
            connection = null;
            Ostrov.log_warn("§cMySql: соединение ostrov сломалось !"+e.getMessage());   //не ставить log_err, или зацикливает!!!
        }*/
        ...
        //try {
            //if ( connection == null || !connection.isValid(3)) {
                ...
                try {
                    ...
                    Ostrov.log_warn("§cMySql: соединение с базой ostrov  не удалось, "+(System.currentTimeMillis()-l)+"мс. -> "+e.getMessage()); //не ставить log_err, или зацикливает!!!
                    connection = null;
                }
            //}
       // } catch (SQLException e) {
       //     connection = null;
        //    Ostrov.log_warn("§cMySql: не удалось соединение с БД ostro: "+e.getMessage());   //не ставить log_err, или зацикливает!!!
        //}
    }*/
