...
public class EntitySetup implements InventoryProvider {
    ...
    public void init(final Player p, final InventoryContent content) {
        ...
        if (en instanceof io.papermc.paper.entity.Shearable) {//(en.getType() == EntityType.SHEEP) {
            io.papermc.paper.entity.Shearable sh = (io.papermc.paper.entity.Shearable) en;
            content.add(ClickableItem.of(new ItemBuilder(sh.readyToBeSheared() ? Material.CLAY_BALL : Material.LIME_DYE)
                    .name(sh.readyToBeSheared() ? "§6Мохнатая" : "§6Стриженная")
                    .build(), e -> {
                if (e.isLeftClick()) {
                    if (sh.readyToBeSheared()) {
                        sh.shear();
                    } else {
//                        sh.setSheared(false); Deprecated
                    }
                    reopen(p, content);
                }
            }));
