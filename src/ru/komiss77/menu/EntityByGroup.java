...
public class EntityByGroup implements InventoryProvider {
    ...
    public void init(final Player p, final InventoryContent contents) {
        ...
        if (radius>0) {
            ...
            for (final Entity e : p.getNearbyEntities(radius, radius, radius)) {
                ...
                if (VM.getNmsEntitygroup().getEntytyGroup(e.getType())==group) {
        ...
        for (final EntityType type : count.keySet()) {
            ...
            menuEntry.add( ClickableItem.of( ItemUtils.buildEntityIcon(type)
                .name("§f"+ (Ostrov.langUtils ? LanguageHelper.getEntityName(type, "RU_ru") : type.toString() ) )
                .lore("§7")
                .lore("§7Найдено: §e"+ count.get(type) )
                .lore("§7")
                ...
                .build(), e -> {
                    if (e.isLeftClick()) {
                        SmartInventory.builder().id("EntityByType"+p.getName()). provider(new EntityByType(world, radius, type)). size(6, 9). title("§2"+world.getName()+", §6"+(Ostrov.langUtils ? LanguageHelper.getEntityName(type, "RU_ru") : type.toString())+", §1r="+radius).build() .open(p);
    ...
    pagination.setItemsPerPage(36);    
        ...
        contents.set(6, 2, new InputButton(new ItemBuilder(Material.FLOWER_BANNER_PATTERN)
